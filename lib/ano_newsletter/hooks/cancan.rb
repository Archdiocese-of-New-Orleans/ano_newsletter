  ...
  module Hooks
    ...
    class CanCanAuthorization
      ...
      def initialize(controller, ability = ::Ability)
        ...
        @controller.current_ability.authorize! :access, :ano_newsletter
      end
      ...
      module ControllerExtension
        def current_ability
          # use ano_newsletter_current_user instead of default current_user so it works with
          # whatever current user method is defined with AnoNewsletter
          @current_ability ||= @ability.new(ano_newsletter_current_user)
        end
      end
    end
  end
...
AnoNewsletter::AUTHORIZATION_ADAPTERS[:cancan] = AnoNewsletter::Hooks::CanCanAuthorization